...
export default function Message({message, messageIndex}) {
    ...
    const image = messageContent.length > 1 ? messageContent[1].image_url.url : null;
    ...
    return (
      <div key={messageIndex} className="message" id={messageIndex} ref={parentDivRef}>
          {/*sender name*/}
          <span className="name">
            {messageContent.role === "user" ? "You: " : "Mysterious traveller: "}
          </span>
          ...
          {text && 
            <div className="smallButtonContainer">
              <CopyButton copyToClipboard={copyToClipboard} copySuccess={copySuccess} text={text}/>

              <button className="smallButton" onClick={() => generateAudio(text)}><AiFillSound /></button>
            </div>
          }
